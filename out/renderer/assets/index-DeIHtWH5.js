const Items = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  get Item() {
    return Item;
  },
...
class Weapon extends Item {
  ...
  speed;
  lifetime;
  constructor(name, game, level = 1, cooldown, damage, speed, lifetime) {
    super(name);
    this.game = game;
    this.level = level;
...
class Enemy extends LivingEntity {
   ...
   * @constructor
   */
  constructor(x, y, maxHp, radius = 12, speed = 200, damage, target) {
    super(x, y, maxHp);
    this.radius = radius;
    this.speed = speed;
    this.damage = damage;
...
class Player extends LivingEntity {
   ...
   * @constructor
   */
  constructor(x, y, maxHp, radius = 16, speed = 200, exp = 0, level = 1, weapons, game) {
    super(x, y, maxHp);
    this.radius = radius;
    this.speed = speed;
    this.exp = exp;
  ...
  gainExp(amount) {
    ...
    while (this.exp >= this.level * 10) {
      ...
      const weaponClasses = Object.values(Items).filter((ItemClass) => {
        const instance = new ItemClass(this.game);
...
class Projectile extends LivingEntity {
   ...
   * @constructor
   */
  constructor(x, y, vx, vy, damage, hp = damage, lifetime, radius, speed) {
    super(x, y, hp);
    this.vx = vx;
    this.vy = vy;
    this.lifetime = lifetime;
...
class KnifeProjectile extends Projectile {
   ...
   * @constructor
   */
  constructor(x, y, vx, vy, damage, speed, lifetime) {
    super(x, y, vx, vy, damage, void 0, lifetime, 5, speed);
  }
  onHit(target) {
...
class TestProjectile extends Projectile {
   ...
   */
  constructor(x, y, vx, vy, damage, speed, lifetime) {
    super(x, y, vx, vy, damage, void 0, lifetime, 7, speed);
  }
  onHit(target) {
    target.hp -= this.damage;
  }
...
class Button extends UIComponent {
  ...
  onClick;
  hover = false;
  constructor(x, y, width, height, text, onClick, canvas) {
    super();
    this.x = x;
    this.y = y;
    this.width = width;
  ...
  handleHover(e) {
    ...
    const mouseX = e.clientX - rect.left;
    const mouseY = e.clientY - rect.top;
    this.hover = mouseX >= this.x && mouseX <= this.x + this.width && mouseY >= this.y && mouseY <= this.y + this.height;
  }
  update(_dt) {
  }
  draw(ctx) {
...
class GameOver extends UIComponent {
  canvas;
  constructor(canvas) {
    super();
    this.canvas = canvas;
...
const App = () => {
  const [_game, setGame] = reactExports.useState(null);
  const [canvas, setCanvas] = reactExports.useState();
  reactExports.useEffect(() => {
    setCanvas(document.getElementById("game"));
  }, []);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setGame(new Game(canvas)), children: "asdf" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { id: "game", width: "1000", height: "800" })
  ] });
};
clientExports.createRoot(document.getElementById("app")).render(
